...
# Install mpi4py and gpustat
RUN pip install mpi4py gpustat

# Install Horovod, temporarily using CUDA stubs
RUN ldconfig /usr/local/cuda-9.0/targets/x86_64-linux/lib/stubs && \
    mkdir /tmp/horovod && \
    cd /tmp/horovod && \
    git clone https://github.com/vloncar/horovod.git && \
    cd horovod && \
    HOROVOD_GPU_ALLREDUCE=NCCL HOROVOD_WITH_TENSORFLOW=1 HOROVOD_WITH_PYTORCH=1 python setup.py install && \
    ldconfig && \
    rm -rf /tmp/horovod
...
RUN mkdir /ml && \
    cd /ml && \
    git clone https://github.com/svalleco/mpi_learn.git && \
    git clone https://github.com/thongonary/mpi_opt.git && \
    cd mpi_opt && \
