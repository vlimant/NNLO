...
class _ConvBlock(nn.Sequential):
    def __init__(self, conv_layers, dropout, in_ch=5):
        ...
        for i in range(conv_layers):
            self.add_module('convlayer%d'%(i), nn.Conv2d(in_ch*((i+1)%5), out_channels=in_ch*((i+2)%5),kernel_size=(3,3),stride=1, padding=1))
            self.add_module('relu%d'%(i), nn.ReLU(inplace=True))
        self.add_module('convlayer%d'%(conv_layers), nn.Conv2d(in_ch*((conv_layers+1)%5), out_channels=1, kernel_size=(3,3), stride=2, padding=1))
...
class _DenseBlock(nn.Sequential):
    def __init__(self, dense_layers, dropout):
        ...
        for i in range(dense_layers):
            self.add_module('denselayer%d'%(i), nn.Linear(10000//(2**i), 10000//(2**(i+1))))
            self.add_module('relu%d'%(i), nn.ReLU(inplace=True))
        self.dropout = dropout
...
class CNN(nn.Module):
    ...
    def build_net(self,*args, **kwargs):
        ...
        self.output = nn.Linear(10000//(2**args[1]), args[3])
